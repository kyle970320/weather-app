...
- **추가/삭제**
  - 홈/상세 페이지에서 현재 보고 있는 위치를 즐겨찾기에 추가/삭제 가능.
...
- **파티클 효과**
  ...
  - 을 캐릭터 뒤 배경에 표시합니다.
- **스켈레톤**
  - 캐릭터 Canvas가 준비되는 동안, 동일 크기의 Skeleton 오버레이를 보여줍니다.

#### 6. 날씨 모드 토글 (WeatherCard)

- WeatherCard 우측 상단에서 모드 토글(예: “기본 / 심플 / 확장” 등)을 통해
  - 날씨가 고정되더라도 캐릭터의 변경 화면을 확인할 수 있도록,
- 내부 로직은 `useChangeWeatherMode` 훅으로 캡슐화되어 위젯 레벨에서 관리됩니다.

#### 7. 에러 페이지 & 404

- **라우트 에러 페이지 (`RouterError`)**
  - React Router의 `errorElement`를 활용해 라우트 렌더링 중 발생하는 에러를  
    공통 에러 UI로 보여줍니다.
...
- **NotFound 페이지**
  - 존재하지 않는 경로(`*`)에 접근 시 404 NotFound 화면을 표시하고  
    “홈으로 돌아가기” 버튼을 제공합니다.
...
- 이유:
  ...
  - `enabled`, `placeholderData`, `staleTime`, `gcTime` 등을 활용해
    - 불필요한 재요청을 줄이고,
...
- 이유:
  - 주소검색(`v2/local/search/address`)과 좌표→주소(`coord2address`)의 baseURL/파라미터가 서로 달라
    **목적별 인스턴스 분리가 가독성과 유지보수에 유리**하기 때문.
...
#### 6. 캐릭터 & 파티클: entity 수준의 3D 모델 + widget 조합
